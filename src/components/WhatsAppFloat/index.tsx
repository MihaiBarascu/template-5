...
export function WhatsAppFloat({
  ...
  // Build WhatsApp URL
  const whatsappUrl = `https://wa.me/${cleanPhone}${defaultMessage ? `?text=${encodeURIComponent(defaultMessage)}` : ''}`
    ...
    >
      ...
      <a
        ...
        className={cn(
          ...
          'bg-[#25D366] hover:bg-[#20BA5C] rounded-full shadow-lg',
          'transition-all duration-300 hover:scale-110',
          'group'
        )}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        aria-label={tooltipText || 'Contact on WhatsApp'}
      >
        {/* Pulse Animation */}
        {pulseAnimation && (
          <>
