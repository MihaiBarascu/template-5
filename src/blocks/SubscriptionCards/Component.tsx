        ...
        <div className="container mx-auto px-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="p-4 text-left border-b border-theme-border">Caracteristici</th>
                  {subscriptions.map((sub) => (
                    <th key={sub.id} className="p-4 text-center border-b border-theme-border">
                      <div className="font-bold text-lg">{sub.title}</div>
                      <div className="text-theme-primary text-2xl font-bold mt-2">
                        {sub.pricing?.amount} {sub.pricing?.currency || 'RON'}
                        <span className="text-sm font-normal text-theme-text-light">{sub.pricing?.period}</span>
                      </div>
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                ...
                {Array.from(
                  new Set(subscriptions.flatMap((sub) => sub.features?.map((f) => f.text) || []))
                ).map((featureText, idx) => (
                  <tr key={idx} className="border-b border-theme-border">
                    <td className="p-4">{featureText}</td>
                    {subscriptions.map((sub) => {
                      const feature = sub.features?.find((f) => f.text === featureText)
                      return (
        ...
        <div className={`grid ${gridClasses[variant]} gap-6 items-stretch`}>
          {subscriptions.map((subscription) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                    ...
                    <h3 className="text-xl font-bold mb-2">{subscription.title}</h3>
                    ...
                    {subscription.cta?.label && (
                      <Link
                        href={subscription.cta.linkType === 'custom' ? (subscription.cta.url || '/contact') : '#'}
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    {isHighlighted && <Star className="w-5 h-5 text-theme-primary fill-current" />}
                    <div>
                      <h3 className="font-bold">{subscription.title}</h3>
                      {subscription.subtitle && (
                        <span className="text-sm text-theme-text-light">{subscription.subtitle}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <span className="text-xl font-bold text-theme-primary">{subscription.pricing?.amount}</span>
                      <span className="text-sm text-theme-text-light ml-1">{subscription.pricing?.currency || 'RON'}{subscription.pricing?.period}</span>
                    </div>
                    <Link
                      href={subscription.cta?.linkType === 'custom' ? (subscription.cta?.url || '/contact') : '#'}
            ...
            return (
              ...
              >
                {/* Highlight label */}
                {isHighlighted && highlightStyle === 'badge' && subscription.highlightLabel && (
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="text-center mb-6">
                    ...
                    {subscription.subtitle && (
                      <p className="text-theme-text-light text-sm">{subscription.subtitle}</p>
                    )}
                  </div>
                  ...
                  <div className="text-center mb-6">
                    <div className="flex items-baseline justify-center gap-1">
                      <span className="text-4xl font-bold text-theme-primary">{subscription.pricing?.amount}</span>
                      <span className="text-theme-text-light">{subscription.pricing?.currency || 'RON'}</span>
                    </div>
                  ...
                  {/* Features */}
                  {showFeatures && subscription.features && subscription.features.length > 0 && (
